...
- **P2SH (Pay-to-Script-Hash)**: Allows outputs to be locked to the hash of a more complex script (like a multisig setup). The sender only needs to send to a standard-looking address, and the receiver reveals the complex script when spending ([BIP 16](https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki)).
- **SegWit (Segregated Witness)**: A major upgrade that separated unlocking data (signatures) from the transaction data, fixing transaction malleability and increasing block capacity ([BIP 141](https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki)).
...
- **P2TR (Pay-to-Taproot)**: The latest upgrade introducing Schnorr signatures and Taproot ([BIP 340](https://github.com/bitcoin/bips/blob/master/bip-0340.mediawiki), [BIP 341](https://github.com/bitcoin/bips/blob/master/bip-0341.mediawiki), [BIP 342](https://github.com/bitcoin/bips/blob/master/bip-0342.mediawiki)). It improves privacy and efficiency by making complex smart contracts look identical to standard single-signature transactions.
...
- **Input [1]**:
  ...
  - **Sequence (`feffffff`)**: Read in little-endian as `fffffffe`. Since this value is less than the max (`ffffffff`), it makes `nLockTime` relevant for this input. It does **not** by itself signal [BIP 125](https://github.com/bitcoin/bips/blob/master/bip-0125.mediawiki) opt-in RBF (that requires `< fffffffe`).
...
This second transaction is a **SegWit** transaction. Because SegWit separates ("segregates") the unlocking data from the transaction input data, the structure of `raw_tx` introduces new fields that were not present in our first example ([BIP 141](https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki)):
...
Now go and spend your time here: [mempool.space](https://mempool.space/).
...
2. **BIP 16 — Pay to Script Hash**. Defines P2SH outputs. [Read BIP 16](https://github.com/bitcoin/bips/blob/master/bip-0016.mediawiki).
3. **BIP 125 — Opt-in Full Replace-by-Fee Signaling**. Defines opt-in RBF signaling rules using `nSequence`. [Read BIP 125](https://github.com/bitcoin/bips/blob/master/bip-0125.mediawiki).
4. **BIP 141 — Segregated Witness (Consensus Layer)**. Introduces SegWit and witness structure. [Read BIP 141](https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki).
5. **BIP 340 — Schnorr Signatures for secp256k1**. Signature scheme used with Taproot. [Read BIP 340](https://github.com/bitcoin/bips/blob/master/bip-0340.mediawiki).
6. **BIP 341 — Taproot: SegWit version 1 spending rules**. Defines Taproot outputs and spending paths. [Read BIP 341](https://github.com/bitcoin/bips/blob/master/bip-0341.mediawiki).
7. **BIP 342 — Validation of Taproot Scripts (Tapscript)**. Defines script validation changes for Taproot spends. [Read BIP 342](https://github.com/bitcoin/bips/blob/master/bip-0342.mediawiki).